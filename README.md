...
`posts` 테이블에 이 컬럼이 필요합니다 (한 번만 실행):
...
동작 방식:
- 관리자 페이지에서 글 작성 중 **임시저장** 버튼을 누르면 `published=false`로 저장되고 편집 화면이 유지됩니다 (공개 여부/예약시각과 무관하게 동작). 관리자 목록에서 "임시저장" 상태로 표시됩니다.
- **공개** 체크를 켜고 **예약 발행 시각**을 미래 시점으로 지정하면, `published=true`이지만 그 시각이 지나기 전까지는 홈 화면/글 목록/상세 페이지 어디서도 노출되지 않습니다 (공개 조회 쿼리 API에 `publish_at is null or publish_at <= now()` 조건이 적용됨). 관리자 목록에서는 "예약 · 날짜"로 표시됩니다.
- 시각을 비워두고 **공개** 체크만 하면 기존과 동일하게 즉시 공개됩니다.

로그인 안 된 상태에서 직접 slug를 추측해도 예약/비공개 글은 조회되지 않습니다 (공개 조회 함수 자체에서 필터링).
